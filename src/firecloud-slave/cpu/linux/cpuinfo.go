...
// CPUInfo 是一个Processor类型的切片(slice),即所有处理器
type CPUInfo struct {
	Processors []Processor `json:"processors"`
}

// NumCPU 方法: 获取CPU逻辑核数量
...
// NumPhysical 方法: 获取CPU物理个数
func (self *CPUInfo) NumPhysical() int {
...
// cpuinfoRegExp:定义正则表达式
var cpuinfoRegExp = regexp.MustCompile("([^:]*?)\\s*:\\s*(.*)$")

// ReadCPUInfo 函数: 读取/proc/cpuinfo文件信息 , 返回CPUInfo结构体
